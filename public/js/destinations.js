...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    if (data.success && data.data.length > 0) {
      container.innerHTML = data.data.map(dest => {
      ...
      container.innerHTML = '<p style="text-align:center; padding: 2rem; color: #666;">No destinations available yet. Check back soon!</p>';
    }
  } catch (error) {
    container.innerHTML = '<p style="text-align:center; padding: 2rem; color: #c94f4f;">Failed to load destinations. Please refresh the page.</p>';
